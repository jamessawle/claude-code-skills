...
function validatePluginFlat(plugin, repoRoot) {
  // Flat format: plugin has "skills" array with paths to skill directories
  ...
  if (!Array.isArray(plugin.skills) || plugin.skills.length === 0) {
    check(`plugin "${plugin.name}" — has skills listed`, () => {
      throw new Error(`No "skills" array or empty skills list`);
    });
    return;
  }

  check(`plugin "${plugin.name}" — has skills listed`, () => {
    // passes — already checked above
  });
  ...
  for (const skillPath of plugin.skills) {
    ...
    check(`plugin "${plugin.name}" — skill directory exists: ${skillPath}`, () => {
      if (!existsSync(skillDir)) {
        throw new Error(`Not found: ${skillDir}`);
      }
    });
...
function run(repoRoot) {
  ...
  for (const plugin of marketplace.plugins) {
    // Detect format: if plugin has a "skills" array, use flat format; otherwise nested
    if (Array.isArray(plugin.skills)) {
      validatePluginFlat(plugin, repoRoot);
    } else {
      validatePluginNested(plugin, repoRoot);
    }
  }
}
